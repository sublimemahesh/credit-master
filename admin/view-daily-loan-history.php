                                            ...
                                            foreach ($INSTALLMENT->getOtherPaymentByLoan($LOAN->id) as $installment_type) {
                                                ...
                                                if ($installment_type['type'] == 'down_payment') {
                                                    ...
                                                    <tr style="background-color: lightsalmon;">     
                                                        ...
                                                        <td class="font-colors text-right f-style"> <?php echo $installment_type['paid_date']; ?></td>   
...
//                                                    echo (-1 * ($amount)) - $last_pay_total + $last_pay_total;
                                                ...
                                                $installment_end->modify('+23 hours +59 minutes +58 seconds');

                                                $installment_end_time = $installment_end->format('Y-m-d H:i:s');

                                                foreach ($INSTALLMENT->CheckInstallmetBeetwenTwoDateByLoanId($date, $installment_end_time, $loan_id) as $Installment_payment) {
                                                    $row_count++;
                                                    ?>
                                                    <tr  id="payment-color">  
                                                        <td>
...
//
                                                                ...
                                                                $od_interest = $LOAN->getOdIntereset1(-$ins_total + $paid_all_amount_before_ins_date1, $od['od_interest_limit']);
...
//receipts between two od dates 
                                                            foreach ($INSTALLMENT->CheckInstallmetBeetwenTwoDateByLoanId($old_od_date, $od_date, $loan_id) as $Installment_payment) {
                                                                $row_count++;
                                                                ?>
                                                                <tr  id="payment-color">  
                                                                    <td>
...
//                                                                            array_push($payment_arr, $payment_balance);
                                                            ...
                                                            if ((-1 * ($od['od_interest_limit'])) < ($balance)) {
                                                                foreach ($INSTALLMENT->CheckInstallmetBeetwenTwoDateByLoanId($od_date, $installment_unlimited_end, $loan_id) as $Installment_payment) {
                                                                    $row_count++;
                                                                    ?>
...
//                                                                            array_push($payment_arr, $payment_balance);
                                                ...
                                                //code end of receipts after installment end 
                                                //
